...
<!-- Konten Edit Produk -->
<h3>Edit Produk</h3>
<form method="post" action="<?= site_url('produk/update/'.$produk->id_produk) ?>" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <select name="id_merek" class="form-control" required>
            <?php foreach ($merek as $m): ?>
                <option value="<?= $m['id_merek'] ?>" <?= ($produk->id_merek == $m['id_merek']) ? 'selected' : '' ?>>
                    <?= $m['nama_merek'] ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-group">
        ...
        <?php if ($produk->foto): ?>
            <img src="<?= base_url('uploads/pembeli/produk/' . $produk->foto) ?>" width="120"><br><br>
        <?php endif; ?>
